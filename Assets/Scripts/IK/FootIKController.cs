...
public class FootIKController : MonoBehaviour
{
    ...
    private void Awake()
    {
        _leftSensor = new RaySensor(Vector3.down);
        _rightSensor = new RaySensor(Vector3.down);
    ...
    private void LateUpdate()
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            if (sensor.Shoot(feetRef.transform.position))
            {
                ...
                feetTargetTransform.SetPositionAndRotation(
                    sensor.GetPoint() + Vector3.up * _footOffset, 
                    Quaternion.FromToRotation(Vector3.up, sensor.GetNormal()) * transform.rotation);
            }
